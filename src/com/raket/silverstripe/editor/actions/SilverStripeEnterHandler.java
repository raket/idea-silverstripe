...
public class SilverStripeEnterHandler extends EnterHandlerDelegateAdapter {
	...
	private static boolean isBetweenHbTags(Editor editor, PsiFile file, int offset) {
		...
		if (chars.charAt(offset - 1) != '>' && chars.charAt(offset - 2) != '%') return false;
		...
		PsiElement openTag = SilverStripePsiUtil.findParentOpenTagElement(openerElement);
		...
		PsiElement closeTag = SilverStripePsiUtil.findParentCloseTagElement(closerElement);
